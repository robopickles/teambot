...
class SlackTextOutput(BaseTextOutput):
    ...
    def send_message(self, message, options):
        ...
        client.chat_postMessage(
            channel=options['slack_channel'], text=message, as_user=True
        )
...
class BaseReportOutput:
    ...
    def format_message(self, users, from_date, to_date):
        if users:
            ...
            context = {'users': formatted, 'from_date': from_date, 'to_date': to_date}
        ...
        attachments = [
            {
                ...
                "pretext": pretext,
                "title": title,
                "title_link": "http://team.gettipsi.com/?from_date={}&to_date={}".format(
                    from_date, to_date
                ),
                "text": markdown,
                "mrkdwn_in": ["text"],
...
class SlackSchedulesStatusOutput:
            ...
            if last:
                if last['status'] == 'failed':
                    self.send_slack_message(
                        client, last, 'danger', 'Scheduled tests have failed', 'FAILED', options
                    )
    ...
    def send_slack_message(self, client: SlackClient, pipeline, color, title, status, options):
        ...
        attachments = [
            {
                ...
                " - {status}".format(
                    pipeline_id=pipeline['id'], project_url=project_url, status=status
                ),
                "color": color,
            }
        ]
