...
from agileapp.views import SummaryParser
...
from .fixtures import get_smon_report, get_upwork_report, load_jira_issue


class TestUpwork(TestCase):
    def setUp(self):
        team = Team.objects.create(name='Default', default=True)
        for upwork_id, username in (['jsmith', 'John Smith'], ['vpupkin', 'Vasya Pupkin']):
            u = UserProfile.objects.create(name=username)
            team.user_profiles.add(u)
            ServiceAccount.objects.create(
                user_profile=u, service_type=ServiceType.upwork, uid=upwork_id
            )

        for issue in ['IOS-96', 'WEB-336', 'IOS-838']:
            Issue.objects.create(
                issue_system=IssueSystem.jira, issue_id=issue, title=issue, description=issue
            )
        self.upwork_patcher = patch(
            'botapp.trackers.UpworkLoader.fetch_team_report', return_value=get_upwork_report()
        )
        self.jira_patcher = patch(
            'botapp.trackers.JiraFetcher.fetch_jira_issue', return_value=load_jira_issue('IOS-153')
        )
    ...
    def test_send_issues_console(self):
        msg = IssueAction().handle_dates(
            date.today(), date.today(), {'sync': True, 'output': 'console'}
        )
        self.assertEqual(
            msg.strip(),
...
class TestSMon(TestCase):
    def setUp(self):
        for smon_user_id, username in (['72690', 'John Smith'], ['72691', 'Vasya Pupkin']):
            u = UserProfile.objects.create(name=username)
            ServiceAccount.objects.create(
                user_profile=u, service_type=ServiceType.smon, uid=smon_user_id
            )
        ...
        self.smon_patcher = patch(
            'botapp.trackers.SMonLoader.fetch_team_report', return_value=get_smon_report()
        )
        self.jira_patcher = patch(
            'botapp.trackers.JiraFetcher.fetch_jira_issue', return_value=load_jira_issue('IOS-153')
        )
...
class TestDateUtils(TestCase):
    def test_normalized_dates(self):
        ...
        assert dates == [
            datetime(2017, 1, 5).date(),
            datetime(2017, 1, 6).date(),
            datetime(2017, 1, 7).date(),
        ], dates
